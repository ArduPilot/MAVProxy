        ...
        while(not self._stop.isSet() ):
            ...
            if(self.heartbeat_ok == True):
                if(self.Condition == ParamStatus.READ_ALL_PARAMETERS):
                    ...
                    valarray = []
                    index = 0;
                    strlen = len(self.mem_area)
                    for index in range(0,15):
                        if(index < strlen):
                            valarray[index] = ord(self.mem_area[index])
                        else:
                            valarray[index] = 0
                    first   = float(valarray[0] + (valarray[1] << 8)) 
                    second  = float(valarray[2] + (valarray[3] << 8))
                    third   = float(valarray[4] + (valarray[5] << 8))
                    fourth  = float(valarray[6] + (valarray[7] << 8))
                    
                    self.mav_proc.mpstate.master().mav.command_long_send(self.mav_proc.sysID, self.mav_proc.compID, mavlink.MAV_CMD_PREFLIGHT_STORAGE_ADVANCED2, 0, mavlink.MAV_PFS_CMD_WRITE_SPECIFIC, 0, first,second,third,fourth,0)
                    ...
                    for index in range(0,15):
                        if(index < strlen):
                            valarray.append(ord(self.mem_area[index])) 
                        else:
                            valarray.append(0)
                    first   = float(valarray[0] + (valarray[1] << 8)) 
                    second  = float(valarray[2] + (valarray[3] << 8))
                    ...
                    self.mav_proc.mpstate.master().mav.command_long_send(self.mav_proc.sysID, self.mav_proc.compID, mavlink.MAV_CMD_PREFLIGHT_STORAGE_ADVANCED2, 0, mavlink.MAV_PFS_CMD_READ_SPECIFIC, 0, first,second,third,fourth,0)

                elif(self.Condition == ParamStatus.CLEAR_MEMORY_AREA):
                    self.Condition = ParamStatus.CLEARING_MEMORY_AREA
                    
       #========================================================================
       #             mem_areas = self.param_handler.get_memory_area_list()
       #             mem_area_str = mem_areas[mem_area]
       #             valarray = []
       #             index = 0;
       #             strlen = len(self.mem_area)
       #             for index in range[0:15]:
       #                 if(index < strlen):
       #                     valarray[index] = ord(mem_area_str[index])
       #                 else:
       #                     valarray[index] = 0
       #             first = valarray[0] + (valarray[1] << 8) + (valarray[2] << 16) + valarray[2] << 24
       #             second = valarray[4] + (valarray[5] << 8) + (valarray[6] << 16) + valarray[7] << 24
       #             third = 0
       #             fourth = 0
       # 
       #             self.mav_proc.mpstate.master().mav.command_long_send(self.mav_proc.sysID, self.mav_proc.compID, mavlink.MAV_CMD_PREFLIGHT_STORAGE_ADVANCED2, 0, mavlink.MAV_PFS_CMD_CLEAR_SPECIFIC, self.mem_area, first,second,third,fourth,0)
       #========================================================================
