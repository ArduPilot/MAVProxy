...
class nv_memory():
    def __init__(self):
        ...
        self.ParamsLoaded = False
        
    def get_memory_area_list(self):
        return self.get_parameter_area_list()      

    def get_parameter_area_list(self):
        area_list = []
        if(self.ParamsLoaded == True):
...
class parameter_handler():
    ...
    def get_nonsync_param_index(self):
        index = 0;
        # return index of the first parameter that is not synchronised
        for param in self.parameters:
            if(param.param_sync == False):
                return index
            index += 1
        # otherwise flag that all parameters are loaded
        self.ParamsLoaded = True
        return -1
