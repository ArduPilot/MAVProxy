...
def init(_mpstate):
  ...
  mpstate.map.add_object(
    mmap.SlipIcon('plane', (0, 0), icon, layer=3, rotation=0,
                  follow=True,
                  trail=mmap.SlipTrail()))
...
def mavlink_packet(m):
  ...
  mpstate.map.set_position(
    'plane',
    (state.lat, state.lon), rotation=state.heading)
  ...
  if state.wp_change_time != mpstate.status.wploader.last_change:
    ...
    if len(points) > 1:
      mpstate.map.add_object(
        mmap.SlipPolygon('mission', points, layer=1, linewidth=2,
                         colour=(255, 255, 255)))
  ...
  if state.fence_change_time != mpstate.status.fenceloader.last_change:
    ...
    if len(points) > 1:
      mpstate.map.add_object(
        mmap.SlipPolygon('fence', points, layer=1, linewidth=2,
                         colour=(0, 255, 0)))
