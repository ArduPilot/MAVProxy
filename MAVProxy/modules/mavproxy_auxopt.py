...
def description():
    ...
    return "auxopt command handling"

def init(_mpstate):
    '''initialise module'''
    global mpstate
    mpstate = _mpstate
    mpstate.command_map['auxopt'] = (cmd_auxopt,   'select option for aux switches on CH7 and CH8 (ArduCopter only)')
    mpstate.completions['auxopt'] = ['set <7|8> <Nothing|Flip|SimpleMode|RTL|SaveTrim|SaveWP|MultiMode|CameraTrigger|Sonar|Fence|ResetYaw|SuperSimpleMode|AcroTrainer|Acro|Auto|AutoTune|Land>',
                                     'reset <7|8|all>',
                                     '<show|list>']
...
def aux_show(channel):
    ...
    opt_num = str(int(mpstate.functions.get_mav_param(param)))
    option = None
...
def cmd_auxopt(args):
    ...
    if args[0] == 'list':
        ...
        if len(args) < 2 or args[1] not in ['7', '8', 'all']:
            ...
            return
        if args[1] == 'all':
            mpstate.functions.param_set('CH7_OPT', '0')
            mpstate.functions.param_set('CH8_OPT', '0')
            return
        param = "CH%s_OPT" % args[1]
        mpstate.functions.param_set(param, '0')
    elif args[0] == 'set':
        ...
        if not option:
            ...
            return
        param = "CH%s_OPT" % args[1]
        mpstate.functions.param_set(param, aux_options[option])
    else:
