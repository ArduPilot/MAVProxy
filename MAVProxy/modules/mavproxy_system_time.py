    ...
    def idle_task(self):
        ...
        if now-self.last_sent > self.system_time_settings.interval:
            ...
            if self.system_time_settings.verbose:
                print("ST: Sending system time: %u" % time_us)
        ...
        if m.get_type() == 'TIMESYNC':
            if m.tc1 == 0:
                ...
                if m.ts1 == self.last_sent_ts1:
                    ...
                    now_ns += 1234
                    print("ST: response to our timesync: sysid=%u latency=%fms" %
                          (m.get_srcSystem(), (now_ns-self.last_sent_ts1)/1000000.0))
