...
TO USE:
    ...
    router dir reset
        set the directory with JSON config files to ../MAVProxy/router_config

    router dir /path/to/dir
        set the directory with JSON config files to /path/to/dir.
        By default, this directory is ../MAVProxy/router_config

    router load my_router
        load the router named my_router.json. To load it, you must create a dict in the file,
        and place it in the selected directory (By default, ../MAVProxy/router_config)

    router save my_router
        save the router in the file my_router.json,
        saved in the selected directory (By default, ../MAVProxy/router_config)
    ...
    def add(self, value: str | list | None):
        ...
        if value == "all":  # We add everything
            ...
            for val in value[4:].split(","):  # Every msg_type that aren't added (i.e. exceptions)
                not_added_values.append(val.upper())
            ...
            if self.allow_all:  # Exceptions = msg_types not forwarded
                ...
                if value not in self.exceptions:
                    self.exceptions.append(value.upper())
    ...
    def remove(self, value: str | list | None):
        ...
        if value == "all":  # We remove everything
            ...
            for val in value[4:].split(","):  # Every msg_types that are added (i.e. exceptions)
                not_removed_values.append(val.upper())
        ...
        if value is None or value == "None" or value == "null":
            ...
            if value == "all":
                ...
                for val in value[4:].split(","):
                    self.exceptions.append(val.upper())
            else:
                self.allow_all = False
                self.exceptions = [value.upper()]
        elif isinstance(value, list):
            self.allow_all = False
            for val in value:
                self.exceptions.append(val.upper())

    def check(self, msg_type : str):
...
class RouterModule(mp_module.MPModule):
    def __init__(self, mpstate):
        ...
        self.router_config_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "router_config")
    ...
    def cmd_router_dir(self, args):
        ...
        with self.router_lock:
            if not args:
                ...
                return
            if args[0] == "reset":
                directory = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "router_config")
            else:
    ...
    def cmd_router_save(self, args):
        ...
        with self.router_lock:
            if self.router_config is None:
                ...
                return
            if "/" in args[0]:
                print("File contains /, impossible to save")
                return
