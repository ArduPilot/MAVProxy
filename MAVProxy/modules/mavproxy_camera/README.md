...
manager. All mount control goes to the flight controller; there is no direct
gimbal control mode or fallback when target transmission is disabled.
...
`gimbal_component` or `manager_gimbal_id`. Other settings
inherit the module defaults. Gimbal association and graphs use the camera's
...
ROI always sends a geographic location to the flight controller, with the
selected mount ID in `MAV_CMD_DO_SET_ROI_LOCATION.param1`. Clearing ROI uses
`MAV_CMD_DO_SET_ROI_NONE.param1`; angle/rate, centre and retract commands use
`MAV_CMD_DO_GIMBAL_MANAGER_PITCHYAW.param7`. These fields match the updated
MAVLink XML. The flight controller replaces its previous target and forwards
geographic ROI to gimbals that support it. MAVProxy does not stream tracking
angles. This requires FC firmware that honours the ROI mount selector.

ROI requests to the same FC are serialized because its `COMMAND_ACK` does not
identify the mount. The next queued mount starts after the current request
completes; different FCs progress independently. Clearing or replacing a pending
request retains its ACK association until completion or timeout. After retries,
cancellation or timeout, queued requests wait for a one-second quiet interval
to drain delayed replies before reusing that manager's ACK association.

MAVProxy never changes `MNTn_TARG_RATE`. Leave it nonzero so ArduPilot can
forward ROI and angle targets. If an older MAVProxy left it at zero, restore
it manually (normally 10 Hz). Disabling it does not switch to direct control.
The obsolete `camera set mount_control manager` command is accepted for startup
script compatibility; `mount_control device` reports an error.
...
## Camera command routing

All MAVLink operations use the existing MAVProxy master link. On ArduPilot,
photo start/stop, recording start/stop, zoom and focus are
addressed to the FC when its cached `CAMn_TYPE=6` and `CAMn_COMPID` parameters
identify exactly one backend for the selected camera. `CAMn_COMPID=0` uses the
usual component `100 + n - 1`. The command carries the one-based camera slot
in its selector: param1 for capture/recording, param3 for zoom/focus.
Mappings use only the selected vehicle's FC parameters; discovery order is
irrelevant. Missing or ambiguous mappings keep the camera component address
so FC routing delivers the command to that camera without guessing a slot.

Source selection is not implemented by ArduPilot's MAVLink camera backend;
mode changes and stream start/stop have no corresponding ArduPilot camera
handler either, so their destination stays the camera component. Discovery, status,
thermal telemetry requests, XML extended parameters and MAVFTP also retain
their component addresses and are routed over the FC link. RTSP/HTTP video and
HTTP definition downloads use the advertised URLs; the FC does not proxy them.
